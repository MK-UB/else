...
/*   Updated: 2023/02/20 12:25:23 by melayoub         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void setup_imgs(t_list *content)
{
	...
	if(!content->bob_down || !content->bob_up || !content->bobleft 
		|| !content->bobright || !content->home || !content-> plancton 
			|| !content->jellyfish || !content->safe || !content->wall || !content->water)
	...
	content->plancton = mlx_xpm_file_to_image(content->mlx_ptr, "./textures/plancton.xpm", &y, &y);
	// content->game_over = mlx_xpm_file_to_image(content->mlx_ptr, "./textures/game_over.xpm", &y, &y);
 	...
 	while(content->map[k])
	{
		...
		while (content->map[k][m])
		{
			...
			if (content->map[k][m] == 'N')
				mlx_put_image_to_window(content->mlx_ptr, content->win_ptr, content->plancton, m * 64, k * 64);
			m++;
        }
        k++;
     }

	mlx_string_put(content->mlx_ptr, content->win_ptr, 0, 0, 0xFFFFFF, "Steps: ");
	mlx_string_put(content->mlx_ptr, content->win_ptr, 70, 0, 0xFFFFFF, c);
	free(c);
}